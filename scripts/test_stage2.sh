...
#SBATCH --export=ALL
#SBATCH --exclude=auh7-1b-gpu-[214-221,260-267],auh7-1b-gpu-259
#SBATCH -w auh7-1b-gpu-[226-227,309-314,249-257]
...
srun --ntasks=${NUM_NODES} --ntasks-per-node=1 bash -lc '
  ...
  torchrun  --nproc_per_node=${GPUS_PER_NODE} --nnodes='"${NUM_NODES}"' --node_rank=${NODE_RANK} --master_addr='"${MASTER_ADDR}"' --master_port='"${MASTER_PORT}"'  /vast/users/guangyi.chen/causal_group/zijian.li/LDM/tabicl_new/tabicl/src/tabicl/train/run.py \
            ...
            --ff_factor 2 \
            --norm_first True \
            --checkpoint_dir /vast/users/guangyi.chen/causal_group/zijian.li/LDM/tabicl_new/tabicl/stage2/checkpoint/dir1_1  \
            --save_temp_every 10 \
